...
# 7. Longest Consecutive Ones in a Query Range


class SegTree:
    def __init__(self,a):
        self.n=len(a)
        self.t=[(0,0,0,0)]*(4*self.n)
        self.build(a,1,0,self.n-1)
    def build(self,a,p,l,r):
        if l==r:
            if a[l]==1:
    ...
    def merge(self,a,b):
        pre1,suf1,best1,len1=a
        pre2,suf2,best2,len2=b
        pre=pre1
        if pre1==len1:
            pre=len1+pre2
        suf=suf2
        if suf2==len2:
            suf=len2+suf1
        best=max(best1,best2,suf1+pre2)
        return (pre,suf,best,len1+len2)
    def update(self,p,l,r,i,x):
        if l==r:
            if x==1:
                self.t[p]=(1,1,1,1)
...
# 10. Bracket Sequence Range Query

class SegTree:
    def __init__(self,s):
        self.n=len(s)
        self.t=[(0,0,0)]*(4*self.n)
        self.build(s,1,0,self.n-1)
    def build(self,s,p,l,r):
        if l==r:
            if s[l]=='(':
                self.t[p]=(1,0,0)
            else:
            ...
            return
        m=(l+r)//2
        self.build(s,p*2,l,m)
        self.build(s,p*2+1,m+1,r)
        self.t[p]=self.merge(self.t[p*2],self.t[p*2+1])
    def merge(self,a,b):
        op1,cl1,pair1=a
        op2,cl2,pair2=b
        x=min(op1,cl2)
        return (op1+op2-x,cl1+cl2-x,pair1+pair2+x)
    def query(self,p,l,r,ql,qr):
        if ql<=l and r<=qr:
            return self.t[p]
        m=(l+r)//2
        ...
        return self.merge(self.query(p*2,l,m,ql,qr),self.query(p*2+1,m+1,r,ql,qr))
    def get(self,l,r):
        return self.query(1,0,self.n-1,l,r)[2]*2
s=input().strip()
st=SegTree(s)
q=int(input())
for _ in range(q):
